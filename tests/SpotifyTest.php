...
class SpotifyTest extends TestCase
{
    ...
    {
        $feed = new \Lukaswhite\FeedWriter\Spotify( );
        $feed->prettyPrint( );
        ...
        $item = $channel->addItem( )
            ...
            ->valid( new \DateTime( '2015-06-22' ), new \DateTime( '2015-07-02' ) )
            ->author( 'John Doe' )
        ...
        $limits = $xpath->query( '/rss/channel/spotify:limit' );
        $this->assertEquals( 1, $limits->length );
        $limitAttributes = $this->getAttributesOfElementNamed( 'spotify:limit', $feed->toString( ) );
        $this->assertArrayHasKey( 'recentCount', $limitAttributes );
        $this->assertEquals( 10, $limitAttributes[ 'recentCount' ] );

        $countries = $xpath->query( '/rss/channel/spotify:countryOfOrigin' );
        $this->assertEquals( 1, $countries->length );
        $this->assertEquals( 'no,se,de', $countries[ 0 ]->textContent );
        ...
        $firstItemValids = $xpath->query( '/rss/channel/item[1]/dcterms:valid' );
        $this->assertEquals( 1, $firstItemValids->length );
        $firstItemValidAttributes = $this->getAttributesOfElementNamed( 'dcterms:valid', $feed->toString( ) );
        $this->assertArrayHasKey( 'start', $firstItemValidAttributes );
        $this->assertEquals( '2015-06-22', substr( $firstItemValidAttributes[ 'start' ], 0, 10 ) );
        $this->assertArrayHasKey( 'end', $firstItemValidAttributes );
        $this->assertEquals( '2015-07-02', substr( $firstItemValidAttributes[ 'end' ], 0, 10 ) );

        $firstItemChapters = $xpath->query( '/rss/channel/item[1]/psc:chapters/psc:chapter' );
        $this->assertEquals( 3, $firstItemChapters->length );

        $chapters = $xpath->query( '/rss/channel/item[1]/psc:chapters/psc:chapter' );
        $this->assertEquals( '0', $chapters[ 0 ]->attributes->getNamedItem( 'start' )->value );
        $this->assertEquals( 'Opening credits', $chapters[ 0 ]->attributes->getNamedItem( 'title' )->value );
        $this->assertEquals( '0:35', $chapters[ 1 ]->attributes->getNamedItem( 'start' )->value );
        $this->assertEquals( 'Today\'s CNN headlines', $chapters[ 1 ]->attributes->getNamedItem( 'title' )->value );
        $this->assertEquals(
            'https://edition.cnn.com/',
            $chapters[ 1 ]->attributes->getNamedItem( 'href' )->value
        );
        $this->assertEquals(
            'https://cdn.cnn.com/cnn/.e/img/3.0/global/misc/cnn-logo.png',
            $chapters[ 1 ]->attributes->getNamedItem( 'image' )->value
        );
        $this->assertEquals( '8:36', $chapters[ 2 ]->attributes->getNamedItem( 'start' )->value );
        $this->assertEquals( 'End credits', $chapters[ 2 ]->attributes->getNamedItem( 'title' )->value );
